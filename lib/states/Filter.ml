...
module Make (Filter : FilterVals) (S : MyMonadicSMemory.S) :
  ...
  let action_from_str s =
    if filter (keep_action s) then S.action_from_str s else None

  let action_to_str a =
    if filter (keep_action (S.action_to_str a)) then S.action_to_str a
    else failwith "Cannot convert hidden action to string"

  let list_preds () =
    List.filter (fun (p, _, _) -> keep_pred (S.pred_to_str p)) (S.list_preds ())

  let pred_from_str s = if filter (keep_pred s) then S.pred_from_str s else None

  let pred_to_str p =
    if filter (keep_pred (S.pred_to_str p)) then S.pred_to_str p
