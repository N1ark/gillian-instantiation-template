...
module Make (S : MyMonadicSMemory.S) : MyMonadicSMemory.S = struct
  ...
  let simplify = function
    | b, None when ExpMap.is_empty b -> None
    | b, n -> Some (b, n)
  ...
  let update_index (s : t option) idx (sub : S.t option) =
    ...
    | Some (b, n), None -> simplify (ExpMap.remove idx b, n)
  ...
  let consume pred (s : t option) ins : (t option * Values.t list, err_t) DR.t =
    let open DR.Syntax in
    let open Delayed.Syntax in
    match (pred, ins) with
    | SubPred p, idx :: ins -> (
        ...
        in
        let** idx, sub = ExpMap.sym_find_res idx b ~err:(MissingCell idx) in
        let+ r = S.consume p (Some sub) ins in
        match r with
        | Ok (sub', outs) -> Ok (update_index s idx sub', outs)
        | Error e -> Error (SubError (idx, e)))
    | SubPred _, [] -> failwith "Missing index for sub-predicate consume"
    | Length, [] -> (
        match s with
        | Some (b, Some n) -> DR.ok (simplify (b, None), [ n ])
        | Some (_, None) | None -> DR.error MissingLength)
